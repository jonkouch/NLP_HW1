...
class FeatureStatistics:
    ...
    def f_101(self, cur_wor, cur_tag) -> int:
        """
        :param cur_wor: current word
        :param cur_tag: current tag
        :return 1 if currrent word ends with 'ing' and t=VBG, 0 otherwise
        """
        if cur_tag == "VBG" and cur_wor.endswith("ing"):
            return 1
        return 0
    
    def f_102(self, cur_wor, cur_tag) -> int:
        """
        :param cur_wor: current word
        :param cur_tag: current tag
        :return 1 if currrent word start with 'pre' and t=NN, 0 otherwise
        """
        if cur_tag == "NN" and cur_wor.endswith("pre"):
            return 1
        return 0
